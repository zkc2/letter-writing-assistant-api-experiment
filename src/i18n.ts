...
export const TRANSLATIONS: Record<Language, TranslationSchema> = {
  en: {
    ...
    chat: {
      ...
      tracker: {
        ...
        items: {
          ...
          experiences: { label: '2. Workplace Context', placeholder: 'Not yet captured' },
          whatToSay: { label: '3. Final Working Date and Handover', placeholder: 'Not yet captured (exact date)' },
          whatNotToSay: { label: '4. Privacy Boundaries', placeholder: 'Begins blank (user decided)' },
        },
      },
      guardrails: {
    ...
    diagram: {
      ...
      trackerSectionTitle: 'Confirmed Case File (0/4 Gate)',
      trackerSectionDesc: 'The Ghostwriter is strictly forbidden from drafting until all 4 inputs are captured and confirmed:',
      trackerItems: {
        t1: { title: '1. Reason for Resigning', desc: 'Core motivation for departure' },
        t2: { title: '2. Workplace Context', desc: 'Unfiltered workplace incidents' },
        t3: { title: '3. Final Working Date and Handover', desc: 'Exact intended final working date and transition commitment' },
        t4: { title: '4. Privacy Boundaries', desc: 'What must NEVER be said in the letter' },
      },
  ...
  zh: {
    ...
    chat: {
      ...
      tracker: {
        ...
        hideChecklist: '收起检查项',
        items: {
          reason: { label: '1. 离职原因', placeholder: '尚未提供' },
          experiences: { label: '2. 职场背景', placeholder: '尚未提供' },
          whatToSay: { label: '3. 最后在岗日与交接', placeholder: '尚未明确（需具体日期）' },
          whatNotToSay: { label: '4. 隐私边界', placeholder: '初始为空（由您自主决定）' },
        },
      },
      guardrails: {
    ...
    diagram: {
      ...
      trackerSectionTitle: '确证案卷门槛 (0/4 Gate)',
      trackerSectionDesc: '信局受严格公文规则约束，在4项要素齐全并经用户确认前，绝不擅自出稿：',
      trackerItems: {
        t1: { title: '1. 离职原因', desc: '离开岗位的核心动因' },
        t2: { title: '2. 职场背景', desc: '真实发生的职场遭遇与阻碍' },
        t3: { title: '3. 最后在岗日与交接', desc: '明确打算在信中传达的事项及确切最后工作日（绝不默认两周）' },
        t4: { title: '4. 隐私边界', desc: '绝不可在信中提及的事项（防范反扑与口实）' },
      },
